angular.module('neon-trends-search').directive('search', function (Connection, $rootScope) {
	return{
		...
		link: function (scope, element, attrs, controllers) {
			var model = {};
			$.getJSON("model/" + $rootScope.connection.database + "." +$rootScope.connection.collection+ ".json", function (data) {
				model = data;
				...
				function search() {
					...
					var queryAds = new neon.query.Query().selectFrom($rootScope.connection.database, $rootScope.connection.collection).where(and(clauses)).limit(5000).sortBy(model.time, neon.query.ASCENDING);
